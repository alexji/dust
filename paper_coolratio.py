...
if __name__ == "__main__":
    ...
    klambda,S = np.load("DATA/std_UM-ND-20_klambda.npy")
    ...
    plt.plot(narr,np.zeros(len(narr))+1.0,'k')
    ...
    for i,T in enumerate(Tarr):
        Gammaarr = Gamma_ad(narr,T)
        linelist[i], = plt.plot(narr, D*Ldarr[:,i]/Gammaarr,plotstyle1[i],lw=2)
        plt.plot(narr, D*Ldarr2[:,i]/Gammaarr,plotstyle2[i],lw=2)

    plt.xlabel(r'n [cm $^{-3}$]',fontsize=16)
    #plt.ylabel(r'$\Lambda_d$ [ergs s$^{-1}$ cm$^{-3}$]',fontsize=16)
    plt.ylabel(r'$\Lambda_{d}/\Gamma_{ad}$',fontsize=16)
    plt.gca().loglog()
    plt.gca().tick_params(axis='both',which='major',labelsize=16)
    plt.gca().set_xticks(10**np.arange(4,19,2))
    #plt.legend(linelist, ['T='+str(x)+'K' for x in Tarr],loc="lower left",prop={'size':18},frameon=False)
    plt.ylim((10**-6,10**2))
    #plt.gcf().set_size_inches(6,12)
    plt.text(10**4.5,30,"standard",fontsize=20)
    plt.savefig("PLOTS/paper_optthickthin_v2a.pdf",bbox_inches="tight")
    ...
    plt.legend(linelist, ['T='+str(x)+'K' for x in Tarr],loc="lower left",prop={'size':18},frameon=False)
    plt.ylim((10**-6,10**2))
    plt.text(10**4.5,30,"shock",fontsize=20)
    plt.savefig("PLOTS/paper_optthickthin_v2b.pdf",bbox_inches="tight")
