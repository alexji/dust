...
##                   'M-D-20', 'M-ND-20', 'M-D-170', 'M-ND-170']
    ...
    rects1 = ax.bar(ind,      std_Td,width,color='g',alpha=0.6,edgecolor='g')
    rects2 = ax.bar(ind+width,x55_Td,width,color='r',alpha=0.6,edgecolor='r')
    plt.ylabel(r'$T_d$ [K]',fontsize=16)
    ax.set_xticks(ind+width)
    ax.set_xticklabels(tuple(dustmodels),rotation=90,size='x-small')

    ax = fig.add_subplot(222)
    rects1 = ax.bar(ind,      std_S,width,color='g',alpha=0.6,edgecolor='g')
    rects2 = ax.bar(ind+width,x55_S,width,color='r',alpha=0.6,edgecolor='r')
    ...
    rects1 = ax.bar(ind,      std_kP,width,color='g',alpha=0.6,edgecolor='g')
    rects2 = ax.bar(ind+width,x55_kP,width,color='r',alpha=0.6,edgecolor='r')
    plt.ylabel(r'$\log_{10}{\kappa_P}$ [cm$^2$/g]',fontsize=16)
    ax.set_xticks(ind+width)
    ax.set_xticklabels(tuple(dustmodels),rotation=90,size='x-small')
    ax.set_ylim((1.5,3.5))
    ax.legend((rects1[0],rects2[0]),('standard','shock'),prop={'size':16},frameon=False)

    ax = fig.add_subplot(224)
    rects1 = ax.bar(ind,      std_Dc,width,color='g',alpha=0.6,edgecolor='g')
    rects2 = ax.bar(ind+width,x55_Dc,width,color='r',alpha=0.6,edgecolor='r')
