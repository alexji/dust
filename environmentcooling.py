...
if __name__ == "__main__":
    ...
    fig = plt.figure()
    fig.subplots_adjust(bottom=.15,wspace=.25,hspace=.3)

    ax  = fig.add_subplot(221)
    rects1 = ax.bar(ind,      std_Td,width,color='r')
    rects2 = ax.bar(ind+width,x55_Td,width,color='y')
    plt.ylabel(r'$T_d$ [K]')
    #ax.set_title("Equilibrium dust temperature")
    ax.set_xticks(ind+width)
    ax.set_xticklabels(tuple(dustmodels),rotation=90,size='xx-small')
    ax.legend((rects1[0],rects2[0]),('std','x5.5'),prop={'size':8})

    ax = fig.add_subplot(222)
    rects1 = ax.bar(ind,      std_S,width,color='r')
    rects2 = ax.bar(ind+width,x55_S,width,color='y')
    plt.ylabel(r'$\log_{10}{S}$ [cm$^2$/g]')
    #ax.set_title("Geometric Cross Section")
    ax.set_xticks(ind+width)
    ax.set_xticklabels(tuple(dustmodels),rotation=90,size='xx-small')
    ax.set_ylim((3,6))

    ax = fig.add_subplot(223)
    rects1 = ax.bar(ind,      std_kP,width,color='r')
    rects2 = ax.bar(ind+width,x55_kP,width,color='y')
    plt.ylabel(r'$\log_{10}{\kappa_P}$ [cm$^2$/g]')
    #ax.set_title(r"Planck mean opacity at $T_d$")
    ax.set_xticks(ind+width)
    ax.set_xticklabels(tuple(dustmodels),rotation=90,size='xx-small')
    ax.set_ylim((1.5,3.5))

    ax = fig.add_subplot(224)
    rects1 = ax.bar(ind,      std_Dc,width,color='r')
    rects2 = ax.bar(ind+width,x55_Dc,width,color='y')
    plt.plot([0,N],[-8.4,-8.4],'k:')
    plt.ylabel(r'$\log_{10}{D_{crit}}$')
    #ax.set_title("Critical dust-to-gas ratio (optically thin)")
    ax.set_xticks(ind+width)
    ax.set_xticklabels(tuple(dustmodels),rotation=90,size='xx-small')
    ax.set_ylim((-9,-6))

    plt.savefig("PLOTS/compare_sizedistr.pdf")
